...
function SetCameraInfoAsymm(channel, fovLeft, fovRight, fovDown, fovUp, near, far, consolidatematrix)
	local ClipTop = math.tan( fovUp * 3.14159/360.0 * 2.0) * near;
	local ClipBottom = -math.tan( fovDown * 3.14159/360.0 * 2.0) * near;
	local ClipLeft = -math.tan(fovLeft * 3.14159/360.0 * 2.0) * near;
	local ClipRight = math.tan(fovRight * 3.14159/360.0 * 2.0) * near;
...
-- Should i be using -VecAud instead of VecAud?
	...
	local matTrans = Matrix4x4_VecTranslateBack(VecAud)
