...
{
  EH_DECLARE;
  EH_Log("derived OM_ToroidScreen::SetUpPropertyAccess()");
...
bool OM_ToroidScreen::CreateGeometry()
{
  ...
  EH_Ptr(vertexCounts = new int[(numRings+1)]);
	// Create enough memory for the vertices and indices
	EH_Ptr(vertexBuffer = new ScreenGeometryBase::SimpleVertex[(maxNumTris * 3)]);
...
void OM_ToroidScreen::createMeshGeometry(ScreenGeometryBase::SimpleVertex *vertexBuffer)
{

  EH_DECLARE;
  EH_Log("Create Toroid:%f %f %f %f %f %f\n", ringStartAngle, ringEndAngle, sideStartAngle, sideEndAngle,
