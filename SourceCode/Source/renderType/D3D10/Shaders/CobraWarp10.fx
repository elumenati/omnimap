...
float CobraWarpWithTrueDimension;
...
struct VS_OUTPUT
{
	...
	float3 PixelPos : COLOR1;				// Pixel position for the alpha mask
...
VS_OUTPUT OmniMapVS(float4 vPos : POSITION)
{
	...
	float  sphericalPhi =  acos(vPos.y/sphericalR ) / 3.14159265359 ;	
	float  sphericalTheta = atan2(-vPos.x,-vPos.z) /3.14159265359;
	...
	Output.pos.xyzw = float4( sphericalTheta *1.5, (-sphericalPhi +.5)*5.5,0,1);
	...
	if(true){
		...
		if(CobraWarpWithTrueDimension < 1.5){
			...
			Output.pos.x =  sphericalTheta  *goalThetaMin ;
			Output.pos.y = sphericalPhi * goalPhiMax + 2.73;
			Output.pos.y = -Output.pos.y;
		
		
		}else{
			float goalThetaMin = 180.0/-75;
			float goalPhiMax= 180.0/-33;
			Output.PixelPos.x = sphericalTheta  *goalThetaMin ;
			Output.PixelPos.y = sphericalPhi * goalPhiMax + 2.73;

			float4 samplepos =  float4(Output.PixelPos.xy *.5 +.5,0,0);
			float4 Offset = tex2Dlod(alphMaskSampler,samplepos).xyzw;

			Offset.xy = (Offset.xy + (Offset.zw / 255.0));
			Output.pos.x = Offset.x*2-1;
			Output.pos.y = Offset.y*2-1;
		}
...
PS_OUTPUT OmniMapFS(VS_OUTPUT In)
{
	...
	for(int i =0; i < NumChannels;i++)	
	{
		...
		if (tc.x >= 0.0 && tc.x <= 1.0 && tc.y >= 0.0 && tc.y <= 1.0 && tc.z > 0.0) {
			tc.y = 1-tc.y;
			Output.RGBColor = MULTISAMPLE_Fin_Fout(Samplers[i], tc.xy);
		}
