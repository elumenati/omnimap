...
#include "omnimap.h"
#include "gltools/simple_matrix.h"
...
/*
	...
	for (int r = 0; r < rows; r++)
	{
		...
		{
			LogSystem()->ReportMessage(" %f", values[r * cols + c]);
		}
		LogSystem()->ReportMessage(" ]");
	}
	*/
...
////////if(true) // new fast way of computing s and t equations....
////////{
	OmniVec3 Eye(0,0,0);//projector?????
	OmniVec3 RayBL(bl.x  - Eye.x,bl.y  - Eye.y,bl.z  - Eye.z);
	OmniVec3 RayBR(br.x  - Eye.x,br.y  - Eye.y,br.z  - Eye.z);
	OmniVec3 RayTL(tl.x  - Eye.x,tl.y  - Eye.y,tl.z  - Eye.z);
	OmniVec3 RayTR(tr.x  - Eye.x,tr.y  - Eye.y,tr.z  - Eye.z);
...
////////
////////
//////// float BL_U = RayBL.dot(RayU)*.5+.5;
//////// float BL_V = RayBL.dot(RayV)*.5+.5;
//////// float BR_U = RayBR.dot(RayU)*.5+.5;
//////// float BR_V = RayBR.dot(RayV)*.5+.5;
////////
//////// float TL_U = RayTL.dot(RayU)*.5+.5;
//////// float TL_V = RayTL.dot(RayV)*.5+.5;
//////// float TR_U = RayTR.dot(RayU)*.5+.5;
//////// float TR_V = RayTR.dot(RayV)*.5+.5;
////////
////////
////////		UPlaneEquation.x = RayU.x;
////////		UPlaneEquation.y = RayU.y;
////////		UPlaneEquation.z = RayU.z;
////////		UPlaneEquation.w = 1;
////////
////////		VPlaneEquation.x = RayV.x;
////////		VPlaneEquation.y = RayV.y;
////////		VPlaneEquation.z = RayV.z;
////////		VPlaneEquation.w = 1;
////////
////////
////////
////////		{{{{{
////////		float predictedP00_u = UPlaneEquation.x * bl.x+	UPlaneEquation.y * bl.y+UPlaneEquation.z *bl.z+UPlaneEquation.w;
////////		float predictedP00_v = VPlaneEquation.x * bl.x+	VPlaneEquation.y * bl.y+VPlaneEquation.z *bl.z+VPlaneEquation.w;
////////
////////		float predictedP01_u = UPlaneEquation.x * tl.x+	UPlaneEquation.y * tl.y+UPlaneEquation.z *tl.z+UPlaneEquation.w;
////////		float predictedP01_v = VPlaneEquation.x * tl.x+	VPlaneEquation.y * tl.y+VPlaneEquation.z *tl.z+VPlaneEquation.w;
////////
////////		float predictedP10_u = UPlaneEquation.x * br.x+	UPlaneEquation.y * br.y+UPlaneEquation.z *br.z+UPlaneEquation.w;
////////		float predictedP10_v = VPlaneEquation.x * br.x+	VPlaneEquation.y * br.y+VPlaneEquation.z *br.z+VPlaneEquation.w;
////////
////////		float predictedP11_u = UPlaneEquation.x * tr.x+	UPlaneEquation.y * tr.y+UPlaneEquation.z *tr.z+UPlaneEquation.w;
////////		float predictedP11_v = VPlaneEquation.x * tr.x+	VPlaneEquation.y * tr.y+VPlaneEquation.z *tr.z+VPlaneEquation.w;
////////
////////
////////		float error =
...
////////	
//////////
//////////	//OmniVec3 RayBL_NONplanar = (RayTL-RayBL).cross(RayBR-RayBL);
//	OmniVec3 RayCenter = RayBL;
//	RayCenter.add(RayBR);
//	RayCenter.add(RayTL);
//	RayCenter.add(RayTR);
//
////	RayCenter.normalize();
//////////
//////////	///(br.x  - Eye.x,br.y  - Eye.y,br.z  - Eye.z); // warning planar...
////////////		OmniVec3 RayBR_NONplanar
//////////
/////////////	RayBR_NONplanar.x *=2.4;
////////////	RayBR_NONplanar.y *=2.4;  // remove planar nature
////////////	RayBR_NONplanar.z *=2.4;
//////////
//////////
//////////
//		float QuadMatrix[4][4]=
//		{		
//			{RayBL.x,RayBL.y,RayBL.z,1},
//			{RayTL.x,RayTL.y,RayTL.z,1},
//			{RayBR.x,RayBR.y,RayBR.z,1},
//			{RayCenter.x,RayCenter.y,RayCenter.z,1.0}
//		};
//
...
//
//		float _Uvalues[4] = {0,0,1,.5};//x s
//		float _Vvalues[4] = {0,1,0,.5};//y t
...
//
//		UPlaneEquation.x = UEquation.getVal(0);
//		UPlaneEquation.y = UEquation.getVal(1);
//		UPlaneEquation.z = UEquation.getVal(2);
//		UPlaneEquation.w = UEquation.getVal(3);
//
//
//		// -.29 x +.624y+.41z-.08
//		VPlaneEquation.x = VEquation.getVal(0);
//		VPlaneEquation.y = VEquation.getVal(1);
//		VPlaneEquation.z = VEquation.getVal(2);
//		VPlaneEquation.w = VEquation.getVal(3);
//
//
//
//		float predictedP00_u = UPlaneEquation.x * RayBL.x+	UPlaneEquation.y * RayBL.y+UPlaneEquation.z *RayBL.z+UPlaneEquation.w;
//		float predictedP00_v = VPlaneEquation.x * RayBL.x+	VPlaneEquation.y * RayBL.y+VPlaneEquation.z *RayBL.z+VPlaneEquation.w;
//
//		float predictedP10_u = UPlaneEquation.x * RayTL.x+	UPlaneEquation.y * RayTL.y+UPlaneEquation.z *RayTL.z+UPlaneEquation.w;
//		float predictedP10_v = VPlaneEquation.x * RayTL.x+	VPlaneEquation.y * RayTL.y+VPlaneEquation.z *RayTL.z+VPlaneEquation.w;
//
//		float predictedP11_u = UPlaneEquation.x * RayTR.x+	UPlaneEquation.y * RayTR.y+UPlaneEquation.z *RayTR.z+UPlaneEquation.w;
//		float predictedP11_v = VPlaneEquation.x * RayTR.x+	VPlaneEquation.y * RayTR.y+VPlaneEquation.z *RayTR.z+VPlaneEquation.w;
//
//		float predictedP01_u = UPlaneEquation.x * RayBR.x+	UPlaneEquation.y * RayBR.y+UPlaneEquation.z *RayBR.z+UPlaneEquation.w;
//		float predictedP01_v = VPlaneEquation.x * RayBR.x+	VPlaneEquation.y * RayBR.y+VPlaneEquation.z *RayBR.z+VPlaneEquation.w;
//
//
//		float error = 
//		(predictedP00_u -0)*(predictedP00_u -0)+
