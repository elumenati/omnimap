...
function SetChannelMatrices(AudPos, ViewDir, HeadsUp)
	if(leftChannel)
	then
		leftChannel:SetViewRotateMatrix4x4(
			CreateViewMatrixFromBasis(AudPos, 
				RotateVec(ViewDir,HeadsUp,45), 
				VectorCopy(HeadsUp) 
			)
		)
		leftChannel:Update()
	...
	if(rightChannel)
	then
		rightChannel:SetViewRotateMatrix4x4(
			CreateViewMatrixFromBasis(AudPos, 
				RotateVec(ViewDir,HeadsUp,-45)  , 
				VectorCopy(HeadsUp) 
			)
	...
	if(bottomChannel)
	then
		bottomChannel:SetViewRotateMatrix4x4(
			CreateViewMatrixFromBasis(AudPos,
				Vector_Reverse(HeadsUp) ,
				RotateVec(ViewDir,HeadsUp,45) 
			)
		)
    ...
    if(topChannel)
	then
		topChannel:SetViewRotateMatrix4x4(
			CreateViewMatrixFromBasis(AudPos,
				VectorCopy(HeadsUp) , 
				RotateVec(Vector_Reverse(ViewDir),HeadsUp,45)
			)
		)
		topChannel:Update()
	end
	...
	if(frontChannel)
	then
		frontChannel:SetViewRotateMatrix4x4(
			CreateViewMatrixFromBasis(AudPos,
				RotateVec(ViewDir,HeadsUp,45+180) ,
				VectorCopy(HeadsUp) 
			)
		)
		frontChannel:Update()
	...
	if(backChannel)
	then
		backChannel:SetViewRotateMatrix4x4(
			CreateViewMatrixFromBasis(AudPos, 
				RotateVec(ViewDir,HeadsUp,-45+180), 
				VectorCopy(HeadsUp)
			)
