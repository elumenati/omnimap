...
void main()
{
	...
	for(int i =0; i < MaxChannels;i++)	
	{
		...
		if(IsInsideSlow(hit))
		{
			gl_FragColor.rgb = texture2D(channelTextures[i], hit).xyz;
//			gl_FragColor.rgb =gl_FragColor.rgb*.1+ .9*DebugIT(Plane_Eq_XYZ[i],Plane_Eq_U[i],Plane_Eq_V[i],userposition, RD);
		}	
	}
