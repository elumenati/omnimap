...
#include "cLua.h"
extern "C" {
#include "lua dist\lua-5.1\src\lua.h"
#include "lua dist\lua-5.1\src\lualib.h"
#include "lua dist\lua-5.1\src\lauxlib.h"
...
// stop compiler from nagging about my typecasting looseness
...
void CWinConsole::Paint(HDC hDC)
{
	...
	{
		TextOut(hDC, x, y, (*it).c_str(), (int) strlen((*it).c_str()));
		y -= 16;
		++it;
	}
}
...
void CWinConsole::SetLogFile(FILE *pLogFile)
{
	if(m_pLogFile)
	{
		fclose(m_pLogFile);
...
extern "C" int _SetLogFile(lua_State *L)
{
	const char *pFilename = luaL_optstring(L, 1, NULL);
	FILE *pLogFile = NULL;
	if(pFilename)
	{
		pLogFile = fopen(pFilename, "wa");
	}
	CWinConsole::SetLogFile(pLogFile);
