...
void OmniMapChannelD3D10::Initialize()
{
	...
	LogSystem()->ReportMessage("OmniMapChannelD3D10::Initialize");
	HRESULT hr;

	ID3D10RenderTargetView* pOrigRT = NULL;
	ID3D10DepthStencilView* pOrigDS = NULL;
	d3dDevice->OMGetRenderTargets( 1, &pOrigRT, &pOrigDS );
	...
	if (multiSampleQuality > 0 || renderTarget == PBUFFER)
	{
		...
		tDesc.SampleDesc.Count = multiSampleQuality;
		...
		tDesc.MiscFlags = 0;
		tDesc.ArraySize = 1;
		hr = d3dDevice->CreateTexture2D( &tDesc, NULL, &renderTargetTexture);

		// Create the render target view
		D3D10_RENDER_TARGET_VIEW_DESC DescRT;
		DescRT.Format = tDesc.Format;
		DescRT.ViewDimension = D3D10_RTV_DIMENSION_TEXTURE2DMS;
		hr = d3dDevice->CreateRenderTargetView( renderTargetTexture, &DescRT, &renderTargetView);
		...
		dstex.SampleDesc.Count = multiSampleQuality;
		UINT pNumQualityLevels;
		hr = d3dDevice->CheckMultisampleQualityLevels(
			DescRTV.Format, (int) multiSampleQuality, &pNumQualityLevels);

		dstex.SampleDesc.Quality = 0; //D3D10_STANDARD_MULTISAMPLE_PATTERN;
		dstex.Usage = D3D10_USAGE_DEFAULT;
		dstex.BindFlags = D3D10_BIND_DEPTH_STENCIL;
		dstex.CPUAccessFlags = 0;
		dstex.MiscFlags = 0;
		dstex.ArraySize = 1;
		hr = d3dDevice->CreateTexture2D( &dstex, NULL, &depthStencilTarget);
		...
		OMNIMAP_V(d3dDevice->CreateDepthStencilView( depthStencilTarget, &DescDS, &depthStencilView));
#ifdef notdef
	} else
	{
		...
		OMNIMAP_V(d3dDevice->CreateTexture2D( &tDesc, NULL, &renderTargetTexture ));
    ...
    OMNIMAP_V( d3dDevice->CreateTexture2D( &Desc, NULL, &channelShdrTexture ) );
    ...
    hr = (d3dDevice->CreateShaderResourceView( channelShdrTexture, &SRVDesc, &renderTargetSRV ));
...
void OmniMapChannelD3D10::beginRenderToChannel()
{
    ...
    ID3D10RenderTargetView* aRTViews[ 1 ] = { renderTargetView };
    d3dDevice->OMSetRenderTargets( 1, aRTViews, depthStencilView );
	D3D10_VIEWPORT vp;
	UINT one = 1;
...
void OmniMapChannelD3D10::CreateHUDSurface()
{
    ...
    hr = d3dDevice->CreateBuffer( &vbdesc, &InitData, &hudVertexBuffer );
