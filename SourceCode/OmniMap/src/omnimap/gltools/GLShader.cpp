...
GLShader::GLShader(const char *filename, const char *prelude, int shaderType)
{
	...
	char *shaderSource;(char *) prelude;
	if (file == NULL)
	{
		shaderSource = new char[strlen(prelude) + 1];
		strcpy(shaderSource, prelude);
	} else {
		shaderSource = new char[strlen(file) + strlen(prelude) + 1];
	strcpy(shaderSource, prelude);
...
GLProgram::use()
{
	...
	if(!checkGLError())
	{
		...
		//LogSystem()->ReportError("Program Log:\n%s", GetLog().c_str());
	}
}
