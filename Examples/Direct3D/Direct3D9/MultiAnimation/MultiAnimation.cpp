...
// Declare a pointer to the OmniMapD3D renderer.
OmniMapD3D *omnimap;
#endif
...
#if defined(USE_OMNIMAP)
	// Create the OmniMapD3D renderer.  Arguments are the window width and height.  Followed by the
	// IDirect3DDevice9 pointer, followed by the startup file for the lua script, and finally the directory
	// containing the generic OmniMap configuration files, and the OmniMap effects file.
	// Implementation specific configuration files are referred to by omnimap_startup.lua, which assumes they are
	// are in ./OmniMapConfig.  This assumption is a run-time assumption and can be changed by modifying 
	// OmniMapConfig/omnimap_startup.lua
	if (omnimap == NULL)
		omnimap = new OmniMapD3D(pBackBufferSurfaceDesc->Width, (FLOAT)pBackBufferSurfaceDesc->Height, 
			pd3dDevice, "OmniMapD3DConfig\\omnimap_startup.lua");
#endif
...
#ifdef USE_OMNIMAP
// Since we do not do the rendering right in OnFrameRender, we need to keep global
// versions of these variables around.  They are set in OnFrameRender.
...
//--------------------------------------------------------------------------------------
#ifdef USE_OMNIMAP
// This is the callback that the OmniMap library will call with the expectation that we will render
// our scene here.  It is called once for each channel for each frame rendered.
void DoRenderPass(OmniMapChannelBase *chan)
{
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
...
#ifndef USE_OMNIMAP
	// If the OmniMapChannel base object is using the FRAME_BUFFER_OBJECT as a rendering target,
	// the channel uses ID3DXRenderToSurface for a rendering surface.  It attaches this surface to a texture
	// with ID3DXRenderToSurface::BeginScene called with a pointer to the texture as an argument,
	// so we do not need to call IDirect3DDevice9::BeginScene here.
	// If you are using the PBUFFER render target, then you must call BeginScene in the application
	// after calling OmniMapChannelBase::beginRenderToChannel.  In OmniMapD3D, the PBUFFER render target option
	// forces the channel to use to use IDirect3DSurface9 as the render target.  So the BeginScene/EndScene calls
	// can be made using a IDirect3DDevice9, which is not possible when using ID3DXRenderToSurface. 
	// To force the channels to use the PBUFFER render surface,
	// set the configuration option renderTarget in OmniMapD3DConfig/omnimap_user_edit.lua to RT_PBUFFER.
	// This is very useful for applications that use toolkits that call BeginScene/EndScene in a method that cannot
	// be overriden by the application programmer.
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        if( g_dwFollow == 0xffffffff )
        {
			...
			// To render different portions of the final scene, OmniMap rotates the camera from
			// the orientation the application intends to draw to the orientation needed for each
			// channel.  Since we are using a shader, that rotation will be ignored, but we still
			// need to let the shader know about it, so we multiply the application's matrix by 
			// the channel rotation matrix to get the desired rotation for this scene.
			// If we were not using a shader, we could just set the view matrix before calling
			// beginRenderToChannel, and the OmniMapAPI would do the rest for us.
			D3DXMATRIX omRotMatrix, rotatedMatrix;
			((OmniMapChannelD3D *) chan)->GetViewMatrix(&omRotMatrix);
			D3DXMatrixMultiply(&rotatedMatrix, &mxView, &omRotMatrix);
			mxView = rotatedMatrix;
#endif

            V( pd3dDevice->SetTransform( D3DTS_VIEW, & mxView ) );
#ifdef USE_OMNIMAP
			// The OmniMapChannelD3D also wants to control projection.  So we override the
			// projection here with the channel's desired projection.
			((OmniMapChannelD3D *) chan)->GetProjectionMatrix(&mxProj);
#endif
        ...
        else
        {
            ...
            D3DXVECTOR3 vAt     ( vCharPos.x, 0.025f, vCharPos.z ),
                        vUp     ( 0.0f, 1.0f, 0.0f );
...
#ifdef USE_OMNIMAP
			// To render each of the channels that comprise the final scene, OmniMap rotates the camera from
			// the orientation the application intends to draw to the orientation needed for each
			// channel.  Since we are using a shader, that rotation will be ignored, but we still
			// need to let the shader know about it, so we multiply the application's matrix by 
			// the channel rotation matrix to get the desired rotation for this scene.
        ...
        {
#ifdef USE_OMNIMAP
			// Only advance the time by a third of elapsed time since we render
			// this 3 times.
			( * itCur )->AdvanceTime( fElapsedTime/3.0, & vEye );
#else
...
#ifdef USE_OMNIMAP
	//  Stop rendering to the current channel.
	chan->endRenderToChannel();
	chanNum++;
#else
	// We defer rendering any overlays until after the PostRender call below in
	// OnFrameRender
        //
...
#ifdef USE_OMNIMAP
// On Frame render saves variables needed for rendering, 
// then calls updates the time for each character. The character timing update is done here so
// that we do not update it 3 times per frame which is what would happen if we did it up in
// DoRenderPass.  Then OmniMapBase::ForEachChannel is called, which causes
// DoRenderPass to be called once for each channel, with a pointer to the channel to be rendered.
// Then PostRender is called to composite the channels to the spherical projection.
// Finally, we render the overlays.
void CALLBACK OnFrameRender( IDirect3DDevice9* _pd3dDevice, double _fTime, float _fElapsedTime, void* _pUserContext )
{
	g_frameNum++;
	HRESULT hr;
	...
	pUserContext = _pUserContext;
	// update time for each tiny
	D3DXVECTOR3 vEye;
	vEye = *g_Camera.GetEyePt();
    vector< CTiny* >::iterator itCur, itEnd = g_v_pCharacters.end();
	omnimap->ForEachChannel(DoRenderPass);
...
void RenderText()
{
	...
	float timeForFrame = DXUTGetGlobalTimer()->GetTime()-g_lastTime;
	if ((DXUTGetGlobalTimer()->GetTime()-g_lastPost) > 1.0) {
		g_frameRate = 1.0/timeForFrame;
		g_frameRate = ((float) (g_frameNum-g_lastFrameCount))/(DXUTGetGlobalTimer()->GetTime()-g_lastPost);
		g_lastPost = DXUTGetGlobalTimer()->GetTime();
		g_lastFrameCount = g_frameNum;
	}
	g_lastTime = DXUTGetGlobalTimer()->GetTime();
	txtHelper.DrawFormattedTextLine( L"  Time: %2.3f FPS:%2.0f", DXUTGetGlobalTimer()->GetTime(), g_frameRate);
...
{
#ifdef USE_OMNIMAP
	// OmniMap will need to recreate itself with the new IDirect3DDevice9, so delete it here,
	// and it will get recreated in OnResetDevice.
	delete(omnimap);
	omnimap = NULL;
#endif
