...
#include "resource.h"
#ifdef USE_OMNIMAP
// Add OmniMapD3D10 include files
#include "OmniMapD3D10.h"
#include "OmniMapChannelD3D10.h"
// Declare the OmniMapD3D10 object
OmniMapD3D10 *omnimap;
#endif
...
HRESULT CALLBACK OnD3D10ResizedSwapChain( ID3D10Device* pd3dDevice, IDXGISwapChain *pSwapChain, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	// Construct the OmniMap object.  The first two arguments are the width and height 
	// of the output window.  The third argument is the lua script file that
	// is used to start the OmniMap configuration process.  The fourth argument
	// is the OmniMap resources folder.
	omnimap = new OmniMapD3D10(pBackBufferSurfaceDesc->Width, pBackBufferSurfaceDesc->Height, 
			pd3dDevice, "OmniMapD3D10Config\\omnimap_startup.lua");
#endif
...
//--------------------------------------------------------------------------------------
#ifdef USE_OMNIMAP
// Declare a structure that can hold data needed by the rendering code.
struct OmniMapCBData {
	ID3D10Device* pd3dDevice; 
	double fTime;
	float fElapsedTime;
} omnimapCBData;
// Declare the method that will be called to render each channel that will
// be composited into the final screen output.
void DoRenderPass(OmniMapChannelBase *chan, void *omniMapCBD)
#else
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
#endif
{
    ...
    HRESULT hr;
#ifdef USE_OMNIMAP
	// Set the variables needed by the rendering code.
	ID3D10Device* pd3dDevice;
	double fTime;
	float fElapsedTime;
	pd3dDevice = ((struct OmniMapCBData *) omniMapCBD)->pd3dDevice;
	fTime = ((struct OmniMapCBData *) omniMapCBD)->fTime;
	fElapsedTime = ((struct OmniMapCBData *) omniMapCBD)->fElapsedTime;
	// Set the Render Target View to be the channel's render target.
	...
	pd3dDevice->OMGetRenderTargets(1, &pRTV, &pDSV);
...
#ifdef USE_OMNIMAP
	// Get the channel's view matrix offset, and projection matrix.
	D3DXMATRIX omRotMatrix, rotatedMatrix;
	((OmniMapChannelD3D10 *) chan)->GetViewMatrix(&omRotMatrix);
	// Offset the view matrix using the channel's view matrix
	D3DXMatrixMultiply(&rotatedMatrix, &mView, &omRotMatrix);
	// Reset the applications view matrix.
	mView = rotatedMatrix;
	...
	((OmniMapChannelD3D10 *) chan)->GetProjectionMatrix(&mProj);
#endif
...
#ifdef USE_OMNIMAP
	// Reset the render target view to that of the application.
	chan->endRenderToChannel();
#else
	// Render the gui after composting the channel's to the screen.
	// (See OnD3D10FrameRender below)
...
#ifdef USE_OMNIMAP
void CALLBACK OnD3D10FrameRender( ID3D10Device* _pd3dDevice, double _fTime, float _fElapsedTime, void* _pUserContext )
{
	// Set the callback data values
	omnimapCBData.pd3dDevice = _pd3dDevice;
	omnimapCBData.fTime = _fTime;
	omnimapCBData.fElapsedTime = _fElapsedTime;
	// Clear the depth buffer.
	ID3D10DepthStencilView* pDSV = DXUTGetD3D10DepthStencilView();
    _pd3dDevice->ClearDepthStencilView( pDSV, D3D10_CLEAR_DEPTH, 1.0, 0 );

	// Render each channel.  This call will cause DoRenderPass to be called once
	// for each channel required by the configuration specified in the lua files.
	// Each channel is rendered into an offscreen dynamic texture.
	omnimap->ForEachChannel(DoRenderPass, (void *) (&omnimapCBData));

	// Composite the channel renderings to the screen with spherical correction.
	omnimap->PostRender();

	// Render the gui after composting the channel's to the screen.
	DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR, L"HUD / Stats" );
    g_HUD.OnRender( _fElapsedTime ); 
    g_SampleUI.OnRender( _fElapsedTime );
    RenderText();
    DXUT_EndPerfEvent();
...
#ifdef USE_OMNIMAP
	// Free memory and resources used by OmniMap.
	delete omnimap;
#endif
    g_Mesh10.Destroy();
}
