...
if(zLeftChannel)
then
	zLeftChannel:CreateChannel() -- last chance to set fbo
	zLeftChannel:InitChannel()   -- last chance to set res
	PositionRenderChannel_In_HudDisplay("zLeft",CreateXY_Rectangle(centerx+scale*1.5+offset+offset,centery,scale,scale)) 
	if(zLeftChannel.UseOmniMapMatrix)
	then
		zLeftChannel:SetViewRotateMatrix4x4(CreateViewMatrixFromBasis(AudiencePos, RotateVec(BaseViewDirection,BaseHeadsUp,-45+180), VectorCopy(BaseHeadsUp)))
        end
	zLeftChannel:Update()
end
...
if(zRightChannel)
then
	zRightChannel:CreateChannel() -- last chance to set fbo
	zRightChannel:InitChannel()   -- last chance to set res
	PositionRenderChannel_In_HudDisplay("zRight",	CreateXY_Rectangle(centerx-scale*1.5-offset-offset,centery,scale,scale))
	if(zRightChannel.UseOmniMapMatrix) 
	then
		zRightChannel:SetViewRotateMatrix4x4(CreateViewMatrixFromBasis(AudiencePos,RotateVec(BaseViewDirection,BaseHeadsUp,45+180) ,VectorCopy(BaseHeadsUp) ))
        end
	zRightChannel:Update()
end
...
if(zLeftChannel)then 		CreateChannelMetaData("zLeft",NumberOfChannels)  NumberOfChannels = NumberOfChannels + 1 end
if(zRightChannel)then 	CreateChannelMetaData("zRight",NumberOfChannels) NumberOfChannels = NumberOfChannels + 1 end
